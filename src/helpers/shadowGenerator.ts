...
const calculateShadow = (depth: number, color = '#000') => {
  if (depth > 24) depth = 24;

  if (depth < 1) return {};

  const shadow = parseShadow(androidDepth.penumbra[depth]);

  const height = shadow.y === 1 ? 1 : Math.floor(shadow.y * 0.5);

  const opacity = interpolate(depth, 1, 24, 0.2, 0.6).toFixed(2);
  const radius = interpolate(shadow.blur, 1, 38, 1, 16).toFixed(2);
  const elevation = depth + 1;
...
export default calculateShadow;
